...
@app.post("/admin/cleanup-dupes")
def cleanup_dupes():
    conn = get_db()
    conn.execute("""
        DELETE FROM guestbook WHERE id NOT IN (
            SELECT MIN(id) FROM guestbook GROUP BY visitor_name, message, created_at
        )
    """)
    conn.execute("""
        DELETE FROM mystery_answers WHERE id NOT IN (
            SELECT MIN(id) FROM mystery_answers GROUP BY question_key, answer, visitor_name, created_at
        )
    """)
    conn.execute("""
        DELETE FROM identity_votes WHERE id NOT IN (
            SELECT MIN(id) FROM identity_votes GROUP BY category, option_text, visitor_name, created_at
        )
    """)
    conn.commit()
    conn.close()
    return {"ok": True, "message": "Duplicates removed"}
